...
const SituationDescriptionsStep: React.FC = () => {
  ...
  const { register, handleSubmit, formState: { errors }, setValue, watch } = useForm<SituationDescriptions>({
  ...
  const handleAIAssistance = async (field: keyof SituationDescriptions) => {
    ...
    try {
      const openAIService = getOpenAIService(apiKey || localStorage.getItem('openai_api_key')!);
      const suggestion = await openAIService.generateSuggestion(
        field,
        currentValues[field]
      );
  ...
  const handleAcceptSuggestion = (text: string) => {
    ...
    toast({
      ...
      description: "The suggestion has been added to your form.",
    });
  };
  ...
  const handleEditSuggestion = (text: string) => {
    ...
    toast({
      ...
      description: "Your edited suggestion has been added to your form.",
    });
  };
  ...
  const fieldConfigs = [
    {
      key: 'currentFinancialSituation' as keyof SituationDescriptions,
      label: t('situation.currentFinancial'),
      placeholder: 'Describe your current financial situation, including income sources, expenses, debts, and any financial difficulties you are experiencing...',
    },
    {
      key: 'employmentCircumstances' as keyof SituationDescriptions,
      label: t('situation.employment'),
      placeholder: 'Describe your employment status, work history, any barriers to employment, and how your current situation affects your ability to work...',
    },
    {
      key: 'reasonForApplying' as keyof SituationDescriptions,
      label: t('situation.reason'),
      placeholder: 'Explain why you are applying for social support, what specific assistance you need, and how it would help improve your situation...',
    },
  ];
          ...
          <div key={key} className="space-y-2">
            ...
            <Textarea
              ...
              {...register(key, { 
                ...
                minLength: { value: 50, message: t('validation.minLength').replace('{0}', '50') }
              })}
